...
The channel is just one receiver (the dark dot) with a bounded buffer of capacity 5 right below it. Green senders type a single char to send: the value flies into the buffer, and sending into a full channel grays that sender until a slot frees.

`Receive` always works — with an empty buffer it simply blocks until a value lands. Each successful receive pulls the oldest char out of the buffer onto the receiver, mirrors it in the label next to `Receive` (FIFO) and unblocks a blocked sender.

Drag the receiver or any sender to rearrange.
...
Each receiver can `.await` (`changed()`) to block until the value differs from what it has seen. Dropping the sender stops all further changes — awaiting receivers just stop, since watch has no error type. The green `Tx`, the `state` cell and every receiver can be dragged to rearrange.
